#!/usr/bin/env bash
# Commit Sprint 66F RBAC manifests to internal Gitea (bootstrap only).
set -euo pipefail
...
upload_file() {
  ...
  if [ -n "$sha" ]; then
    method=PUT
    payload="{\"message\":\"Sprint 66F: update $dest\",\"content\":\"$b64\",\"sha\":\"$sha\"}"
  else
    method=POST
    payload="{\"message\":\"Sprint 66F: add $dest\",\"content\":\"$b64\"}"
  fi
...
for f in serviceaccount.yaml role.yaml rolebinding.yaml; do
  upload_file "$ROOT/deploy/pilot-internal/k8s/infra/pilot-demo/rbac/$f" "infra/pilot-demo/rbac/$f"
done
echo "gitea_rbac_commit_complete"
