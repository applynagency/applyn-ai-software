...
class FrontendDeveloperV3Agent:
    def __init__(self):
        ...
        self.client = gateway_anthropic_client(feature="agent:frontend_v3")
        self.prompt_builder = FrontendDeveloperV3PromptBuilder()

    async def run(
        self,
        *,
        requirement_text: str,
        frontend_v2_output: dict,
    ) -> tuple[FrontendDeveloperV3Output, int]:
        logger.info("frontend_v3_agent_start", requirement_length=len(requirement_text))

        user_prompt = self.prompt_builder.build_user_prompt(
            requirement_text=requirement_text,
            frontend_v2_output=frontend_v2_output,
        )
    ...
    def _parse_output(self, data: dict) -> FrontendDeveloperV3Output:
        return FrontendDeveloperV3Output(
            generated_files=self._parse_generated_files(data.get("generated_files", [])),
            project_structure=data.get("project_structure") or {},
            package_json=data.get("package_json") or {},
