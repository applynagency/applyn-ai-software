...
class DockerAgent:
    def __init__(self):
        ...
        self.client = gateway_anthropic_client(feature="agent:docker_agent")
    ...
    async def run(
        ...
        *,
    ...
    ) -> tuple[DockerAgentOutput, int]:
        logger.info("docker_agent_start", requirement_length=len(requirement_text))
        ...
        logger.info("docker_agent_complete", tokens_used=tokens_used)
        return output, tokens_used

    def _parse_output(self, data: dict) -> DockerAgentOutput:
        return DockerAgentOutput(
            dockerfile_strategy=data.get("dockerfile_strategy") or "",
            docker_compose=data.get("docker_compose") or "",
            container_topology=data.get("container_topology") or "",
            runtime_configuration=data.get("runtime_configuration") or "",
            image_optimization=data.get("image_optimization") or "",
            security_hardening=data.get("security_hardening") or "",
        )
