import json
import re
import uuid

import anthropic

from app.core.config import settings
from app.core.exceptions import AgentError
from app.core.logging import get_logger
from app.schemas.unit_test import TestFixture, UnitTestGeneratorOutput, UnitTestSpecification
from app.unit_test_generator.prompt_builder import UnitTestGeneratorPromptBuilder

logger = get_logger(__name__)


class UnitTestGeneratorAgent:
    def __init__(self):
        if not settings.ANTHROPIC_API_KEY:
            raise AgentError("ANTHROPIC_API_KEY is not configured. Add it in the Secrets panel.")
        from app.ai.compat import gateway_anthropic_client

        self.client = gateway_anthropic_client(feature="agent:unit_test_generator")
        self.prompt_builder = UnitTestGeneratorPromptBuilder()

    async def run(
        self,
        *,
        requirement_text: str,
        qa_architect_output: dict,
    ) -> tuple[UnitTestGeneratorOutput, int]:
        logger.info("unit_test_generator_agent_start", requirement_length=len(requirement_text))

        user_prompt = self.prompt_builder.build_user_prompt(
            requirement_text=requirement_text,
            qa_architect_output=qa_architect_output,
        )

        try:
            message = await self.client.messages.create(
                model=settings.ANTHROPIC_MODEL,
                max_tokens=settings.ANTHROPIC_MAX_TOKENS,
                system=self.prompt_builder.get_system_prompt(),
                messages=[{"role": "user", "content": user_prompt}],
            )
        except anthropic.AuthenticationError as exc:
            raise AgentError("Invalid ANTHROPIC_API_KEY — check your Secrets panel.") from exc
        except anthropic.RateLimitError as exc:
            raise AgentError("Anthropic rate limit hit — please retry in a moment.") from exc
        except Exception as exc:
            raise AgentError(f"LLM call failed: {str(exc)}") from exc

        tokens_used = (message.usage.input_tokens + message.usage.output_tokens) if message.usage else 0
        raw_content = ""
        for block in message.content:
            if block.type == "text":
                raw_content = block.text
                break

        if not raw_content:
            raise AgentError("Empty response from Claude")

        raw_content = re.sub(r"^```(?:json)?\s*", "", raw_content.strip())
        raw_content = re.sub(r"\s*```$", "", raw_content.strip())

        try:
            data = json.loads(raw_content)
        except json.JSONDecodeError as exc:
            raise AgentError(f"Failed to parse Claude's response as JSON: {str(exc)}") from exc

        try:
            output = self._parse_output(data)
        except Exception as exc:
            raise AgentError(f"Failed to structure agent output: {str(exc)}") from exc

        logger.info("unit_test_generator_agent_complete", tokens_used=tokens_used)
        return output, tokens_used

    def _parse_output(self, data: dict) -> UnitTestGeneratorOutput:
        def uid(prefix: str) -> str:
            return f"{prefix}-{str(uuid.uuid4())[:6]}"

        def parse_specs(items: list, prefix: str) -> list[UnitTestSpecification]:
            return [
                UnitTestSpecification(
                    id=item.get("id") or uid(prefix),
                    name=item.get("name", ""),
                    description=item.get("description", ""),
                    target_module=item.get("target_module", ""),
                    test_type=item.get("test_type", "unit"),
                    assertions=item.get("assertions") or [],
                )
                for item in items
            ]

        return UnitTestGeneratorOutput(
            frontend_unit_test_specifications=parse_specs(
                data.get("frontend_unit_test_specifications") or [], "FE-UT"
            ),
            backend_unit_test_specifications=parse_specs(
                data.get("backend_unit_test_specifications") or [], "BE-UT"
            ),
            mock_strategy=data.get("mock_strategy") or {},
            test_fixtures=[
                TestFixture(
                    id=item.get("id") or uid("FIX"),
                    name=item.get("name", ""),
                    description=item.get("description", ""),
                    setup=item.get("setup"),
                    teardown=item.get("teardown"),
                )
                for item in data.get("test_fixtures") or []
            ],
            coverage_targets=data.get("coverage_targets") or {},
        )
