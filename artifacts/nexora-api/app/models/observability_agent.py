...
class ObservabilityRunStatus(str, enum.Enum):
    PENDING = "PENDING"
    RUNNING = "RUNNING"
    COMPLETED = "COMPLETED"
    FAILED = "FAILED"
...
class ObservabilityRun(Base, UUIDMixin, TimestampMixin):
    ...
    status: Mapped[ObservabilityRunStatus] = mapped_column(
        String(20), nullable=False, default=ObservabilityRunStatus.PENDING
    )
    ...
    artifacts: Mapped[list["ObservabilityArtifact"]] = relationship(
        "ObservabilityArtifact", back_populates="run", cascade="all, delete-orphan"
    )


class ObservabilityArtifact(Base, UUIDMixin, TimestampMixin):
    __tablename__ = "observability_artifacts"
    ...
    run: Mapped["ObservabilityRun"] = relationship("ObservabilityRun", back_populates="artifacts")
