...
class SecurityTestRunStatus(str, enum.Enum):
    PENDING = "PENDING"
    RUNNING = "RUNNING"
    COMPLETED = "COMPLETED"
    FAILED = "FAILED"
...
class SecurityTestRun(Base, UUIDMixin, TimestampMixin):
    ...
    status: Mapped[SecurityTestRunStatus] = mapped_column(
        String(20), nullable=False, default=SecurityTestRunStatus.PENDING
    )
    ...
    artifacts: Mapped[list["SecurityTestArtifact"]] = relationship(
        "SecurityTestArtifact", back_populates="run", cascade="all, delete-orphan"
    )


class SecurityTestArtifact(Base, UUIDMixin, TimestampMixin):
    __tablename__ = "security_test_artifacts"

    run_id: Mapped[str] = mapped_column(
        String(36), ForeignKey("security_test_runs.id", ondelete="CASCADE"), nullable=False, index=True
    )
    ...
    run: Mapped["SecurityTestRun"] = relationship("SecurityTestRun", back_populates="artifacts")
