...
async def test_run_frontend_execution_success(client):
    ...
    response = await run_frontend_execution(client, tokens["access_token"], requirement["id"])
...
async def test_run_requires_frontend_code_review_first(client):
    _, tokens = await create_authenticated_user(
...
async def test_get_frontend_execution_run(client):
    _, tokens = await create_authenticated_user(
    ...
    response = await client.get(
        f"/v1/agents/frontend-execution/runs/{run_id}",
...
async def test_get_frontend_execution_artifact(client):
    _, tokens = await create_authenticated_user(
    ...
    response = await client.get(
        f"/v1/agents/frontend-execution/artifacts/{artifact_id}",
        headers=auth_headers(tokens["access_token"]),
    )
    assert response.status_code == 200
...
async def test_list_runs_by_requirement(client):
    ...
    response = await client.get(
        f"/v1/agents/frontend-execution/{requirement['id']}",
        headers=auth_headers(tokens["access_token"]),
    )
    assert response.status_code == 200
...
async def test_artifact_has_markdown(client):
    ...
    await setup_frontend_execution_pipeline(client, tokens["access_token"], requirement["id"])
    created = await run_frontend_execution(client, tokens["access_token"], requirement["id"])
    assert "Frontend Execution Report" in created.json()["artifact"]["artifact_markdown"]
...
async def test_run_stores_frontend_v3_and_code_review_run_ids(client):
    _, tokens = await create_authenticated_user(
...
async def test_run_not_found(client):
    ...
    response = await client.get(
        "/v1/agents/frontend-execution/runs/missing-id",
        headers=auth_headers(tokens["access_token"]),
    )
...
async def test_artifact_not_found(client):
    ...
    response = await client.get(
        "/v1/agents/frontend-execution/artifacts/missing-id",
        headers=auth_headers(tokens["access_token"]),
    )
    assert response.status_code == 404
