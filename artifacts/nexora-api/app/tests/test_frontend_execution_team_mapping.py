...
async def test_product_team_includes_frontend_execution_mapping(client):
    _, tokens = await create_authenticated_user(
        client, email="map-product-fe@example.com", username="mapproductfe"
    )
...
async def test_frontend_team_mapping(client):
    _, tokens = await create_authenticated_user(
        client, email="map-frontend-fe@example.com", username="mapfrontendfe"
    )
