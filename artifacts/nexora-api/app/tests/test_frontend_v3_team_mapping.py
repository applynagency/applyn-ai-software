...
async def test_product_team_includes_frontend_v3_mapping(client):
    _, tokens = await create_authenticated_user(
        client, email="map-product-fv3@example.com", username="mapproductfv3"
    )
...
async def test_frontend_team_mapping(client):
    _, tokens = await create_authenticated_user(
        client, email="map-frontend-fv3@example.com", username="mapfrontendfv3"
    )
