...
async def test_product_team_includes_frontend_v2_mapping(client):
    _, tokens = await create_authenticated_user(
        client, email="map-product-fv2@example.com", username="mapproductfv2"
    )
...
async def test_frontend_team_mapping(client):
    _, tokens = await create_authenticated_user(
        client, email="map-frontend-fv2@example.com", username="mapfrontendfv2"
    )
