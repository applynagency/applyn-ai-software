...
async def test_backend_v3_started_audit(client):
    ...
    assert "backend_v3_started" in actions


async def test_backend_v3_completed_audit(client):
    from app.database.session import AsyncSessionLocal
    ...
    async with AsyncSessionLocal() as session:
        result = await session.execute(select(AuditLog).where(AuditLog.resource_id == run_id))
        actions = {log.action for log in result.scalars().all()}
    assert "backend_v3_completed" in actions
    assert "backend_v3_validated" in actions


async def test_backend_v3_validated_audit_has_score(client):
    from app.database.session import AsyncSessionLocal
    ...
    async with AsyncSessionLocal() as session:
        result = await session.execute(
            select(AuditLog).where(
                ...
                AuditLog.action == "backend_v3_validated",
            )
        )
