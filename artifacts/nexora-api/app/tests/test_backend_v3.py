...
async def test_run_backend_v3_success(client):
...
async def test_run_requires_backend_v2_first(client):
    _, tokens = await create_authenticated_user(
...
async def test_get_backend_v3_run(client):
    _, tokens = await create_authenticated_user(
...
async def test_get_backend_v3_artifact(client):
    _, tokens = await create_authenticated_user(
...
async def test_artifact_has_markdown(client):
    ...
    await setup_backend_v3_pipeline(client, tokens["access_token"], requirement["id"])
    created = await run_backend_v3(client, tokens["access_token"], requirement["id"])
    assert "Generated Backend Codebase" in created.json()["artifact"]["artifact_markdown"]
...
async def test_run_stores_backend_v2_run_id(client):
    _, tokens = await create_authenticated_user(
    ...
    pipeline = await setup_backend_v3_pipeline(client, tokens["access_token"], requirement["id"])
    created = await run_backend_v3(client, tokens["access_token"], requirement["id"])
    assert created.json()["backend_v2_run_id"] == pipeline["backend_v2_run"]["id"]
...
async def test_run_not_found(client):
    ...
    response = await client.get(
        "/v1/agents/backend-v3/runs/missing-id",
        headers=auth_headers(tokens["access_token"]),
    )
...
async def test_artifact_not_found(client):
    ...
    response = await client.get(
        "/v1/agents/backend-v3/artifacts/missing-id",
        headers=auth_headers(tokens["access_token"]),
    )
    assert response.status_code == 404
