...
async def test_product_team_includes_frontend_v2():
    ...
    assert "frontend_v2" in agents
    assert agents.index("frontend_v1") < agents.index("frontend_v2")


async def test_frontend_team_includes_frontend_v2():
    from app.models.team import TeamType
    from app.teams.mappings import TeamMappingService

    agents = TeamMappingService.agents_for_team_type(TeamType.FRONTEND)
    assert agents[1] == "frontend_v1"
    assert agents[2] == "frontend_v2"


async def test_workflow_execution_runs_full_pipeline_including_frontend_v2(client):
    _, tokens = await create_authenticated_user(
        client, email="fv2-exec@example.com", username="fv2exec"
    )
...
async def test_execution_plan_marks_frontend_v2_implemented(client):
    _, tokens = await create_authenticated_user(
        client, email="fv2-plan@example.com", username="fv2plan"
    )
    ctx = await setup_execution_context(client, tokens["access_token"])
    with patch_product_owner_agent(), patch_business_analyst_agent():
        response = await client.post(
    ...
    fv2 = next(a for a in agents if a.get("internal_agent") == "frontend_v2")
    assert fv2["is_implemented"] is True


async def test_frontend_v2_execution_order_after_frontend_v1(client):
    _, tokens = await create_authenticated_user(
        client, email="fv2-order@example.com", username="fv2order"
    )
    ctx = await setup_execution_context(client, tokens["access_token"])
    with patch_product_owner_agent(), patch_business_analyst_agent():
        response = await client.post(
    ...
    assert fv1["execution_order"] < fv2["execution_order"]
