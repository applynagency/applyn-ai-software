...
from app.security_test.validator import MINIMUM_COUNTS, SecurityTestValidator
...
def test_valid_output_passes(validator):
    result = validator.validate(mock_security_test_output())
...
def test_empty_output_fails(validator):
    result = validator.validate(SecurityTestOutput())
...
def _build_items(field: str, count: int):
    return [
        SecurityAssessmentItem(
            ...
            severity="high", recommendation="patch"
        )
