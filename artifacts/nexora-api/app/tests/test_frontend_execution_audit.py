...
async def test_frontend_execution_started_audit(client):
    ...
    assert "frontend_execution_started" in actions


async def test_frontend_execution_completed_audit(client):
    from app.database.session import AsyncSessionLocal
    ...
    async with AsyncSessionLocal() as session:
        result = await session.execute(select(AuditLog).where(AuditLog.resource_id == run_id))
        actions = {log.action for log in result.scalars().all()}
    assert "frontend_execution_completed" in actions


async def test_frontend_execution_failed_audit(client):
    _, tokens = await create_authenticated_user(
...
async def test_frontend_execution_approved_audit(client):
    from app.database.session import AsyncSessionLocal
    ...
    async with AsyncSessionLocal() as session:
        result = await session.execute(
            select(AuditLog).where(
                ...
                AuditLog.action == "frontend_execution_approved",
            )
        )
        log = result.scalars().first()
    assert log is not None
    assert log.details["approval_status"] == "FRONTEND_APPROVED"
