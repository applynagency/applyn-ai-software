...
async def test_execution_logs_present_in_artifact(client):
    ...
    assert len(logs) >= 1
    assert any("npm install" in log for log in logs)
...
async def test_step_results_present_in_artifact(client):
    ...
    created = await run_frontend_execution(client, tokens["access_token"], requirement["id"])
    artifact = created.json()["artifact"]["artifact_json"]
    assert artifact["lint_results"]["status"] == "success"
    assert artifact["build_results"]["status"] == "success"
...
async def test_approval_status_stored_on_run(client):
    ...
    await setup_frontend_execution_pipeline(client, tokens["access_token"], requirement["id"])
    created = await run_frontend_execution(client, tokens["access_token"], requirement["id"])
    assert created.json()["approval_status"] == "FRONTEND_APPROVED"
...
async def test_install_results_present_in_artifact(client):
    _, tokens = await create_authenticated_user(
    ...
    await setup_frontend_execution_pipeline(client, tokens["access_token"], requirement["id"])
    created = await run_frontend_execution(client, tokens["access_token"], requirement["id"])
    install = created.json()["artifact"]["artifact_json"]["install_results"]
