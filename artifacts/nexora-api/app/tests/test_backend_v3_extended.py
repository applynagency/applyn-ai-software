...
async def test_requirements_txt_present_in_artifact(client):
    _, tokens = await create_authenticated_user(
    ...
    await setup_backend_v3_pipeline(client, tokens["access_token"], requirement["id"])
    created = await run_backend_v3(client, tokens["access_token"], requirement["id"])
    requirements_txt = created.json()["artifact"]["artifact_json"]["requirements_txt"]
...
async def test_docker_configuration_present_in_artifact(client):
    ...
    assert docker.get("base_image") == "python:3.11-slim"
...
async def test_readme_present_in_artifact(client):
    ...
    assert "Generated Backend" in readme
...
async def test_project_structure_present_in_artifact(client):
    ...
    assert structure.get("framework") == "fastapi"
