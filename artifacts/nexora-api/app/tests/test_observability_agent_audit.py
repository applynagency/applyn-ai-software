...
async def test_observability_started_audit(client):
    run_id = await _run(client, "obs-audit@example.com", "obsaudit", "obs-aud")
    actions = await _actions_for(run_id)
    assert "observability_started" in actions


async def test_observability_completed_audit(client):
    run_id = await _run(client, "obs-aud-done@example.com", "obsauddn", "obs-aud-done")
    actions = await _actions_for(run_id)
    assert "observability_completed" in actions


async def test_observability_validated_audit(client):
    run_id = await _run(client, "obs-aud-val@example.com", "obsaudvl", "obs-aud-val")
    actions = await _actions_for(run_id)
    assert "observability_validated" in actions
