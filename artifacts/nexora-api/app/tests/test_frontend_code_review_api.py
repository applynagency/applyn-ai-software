...
async def test_post_run_returns_201(client):
    _, tokens = await create_authenticated_user(
        client, email="fcr-api-post@example.com", username="fcrapipost"
    )
    workspace = await create_workspace(client, tokens["access_token"], slug="fcr-api-ws")
    project = await create_project(
        client, tokens["access_token"], workspace_id=workspace["id"], slug="fcr-api-proj"
    )
...
async def test_get_runs_endpoint_returns_list(client):
    _, tokens = await create_authenticated_user(
        client, email="fcr-api-list@example.com", username="fcrapilist"
    )
    workspace = await create_workspace(client, tokens["access_token"], slug="fcr-list-api-ws")
    project = await create_project(
        client, tokens["access_token"], workspace_id=workspace["id"], slug="fcr-list-api-proj"
    )
    requirement = await create_requirement(
        client, tokens["access_token"], project_id=project["id"]
    )
...
async def test_artifact_endpoint_returns_json_and_markdown(client):
    _, tokens = await create_authenticated_user(
        client, email="fcr-api-art@example.com", username="fcrapiart"
    )
    workspace = await create_workspace(client, tokens["access_token"], slug="fcr-art-api-ws")
    project = await create_project(
        client, tokens["access_token"], workspace_id=workspace["id"], slug="fcr-art-api-proj"
    )
    requirement = await create_requirement(
        client, tokens["access_token"], project_id=project["id"]
    )
    ...
    assert body["review_score"] is not None
