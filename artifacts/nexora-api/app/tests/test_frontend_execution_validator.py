...
def test_validator_accepts_complete_output():
    validator = FrontendExecutionValidator()
    result = validator.validate(mock_frontend_execution_output())
...
def test_validator_rejects_missing_build_status():
    validator = FrontendExecutionValidator()
    output = FrontendExecutionOutput.model_construct(
        build_status=None,
...
def test_validator_rejects_invalid_build_status():
    validator = FrontendExecutionValidator()
    output = mock_frontend_execution_output(build_status="unknown")
    result = validator.validate(output)
    assert result.is_valid is False
    assert any("build_status" in error for error in result.errors)
...
def test_validator_rejects_invalid_approval_status():
    validator = FrontendExecutionValidator()
    output = mock_frontend_execution_output(approval_status="INVALID_STATUS")
    result = validator.validate(output)
    assert result.is_valid is False
    assert any("approval_status" in error for error in result.errors)
...
def test_validator_rejects_empty_execution_logs():
    validator = FrontendExecutionValidator()
    output = mock_frontend_execution_output(execution_logs=[])
    result = validator.validate(output)
    assert result.is_valid is False
    assert any("execution_logs" in error for error in result.errors)
...
def test_validator_rejects_missing_build_results_on_success():
    validator = FrontendExecutionValidator()
    output = mock_frontend_execution_output(build_status="success", build_results={})
    result = validator.validate(output)
    assert result.is_valid is False
    assert any("build_results" in error for error in result.errors)


def test_validator_accepts_failed_build_without_build_results():
    validator = FrontendExecutionValidator()
...
def test_validator_counts_are_reported():
    ...
    assert result.counts["lint_status"] == "success"


def test_validator_accepts_all_valid_approval_statuses():
    validator = FrontendExecutionValidator()
    for status in FrontendExecutionApprovalStatus:
        output = mock_frontend_execution_output(approval_status=status.value)
        result = validator.validate(output)
