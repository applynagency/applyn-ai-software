...
async def test_developer_can_run_backend_execution(client):
    ...
    response = await run_backend_execution(client, dev_tokens["access_token"], requirement["id"])
    assert response.status_code == 201


async def test_developer_can_view_backend_execution_run(client):
    owner, owner_tokens = await create_authenticated_user(
    ...
    response = await client.get(
        f"/v1/agents/backend-execution/runs/{created.json()['id']}",
        headers=auth_headers(dev_tokens["access_token"]),
    )
    assert response.status_code == 200


async def test_viewer_cannot_run_backend_execution(client):
    owner, owner_tokens = await create_authenticated_user(
    ...
    response = await run_backend_execution(
        client, viewer_tokens["access_token"], requirement["id"]
    )
    assert response.status_code == 403


async def test_project_manager_can_run_backend_execution(client):
    owner, owner_tokens = await create_authenticated_user(
    ...
    await setup_backend_execution_pipeline(client, pm_tokens["access_token"], requirement["id"])
    response = await run_backend_execution(client, pm_tokens["access_token"], requirement["id"])
    assert response.status_code == 201
