...
async def test_developer_can_run_backend_v3(client):
    ...
    await setup_backend_v3_pipeline(client, owner_in_org["access_token"], requirement["id"])
    await client.post(
    ...
    response = await run_backend_v3(client, dev_tokens["access_token"], requirement["id"])
    assert response.status_code == 201


async def test_developer_can_view_backend_v3_run(client):
    owner, owner_tokens = await create_authenticated_user(
    ...
    await setup_backend_v3_pipeline(client, owner_in_org["access_token"], requirement["id"])
    created = await run_backend_v3(client, owner_in_org["access_token"], requirement["id"])
    await client.post(
        f"/v1/organizations/{organization['id']}/members",
        headers=auth_headers(owner_tokens["access_token"]),
        json={"user_id": developer["id"], "role": "DEVELOPER"},
    ...
    response = await client.get(
        f"/v1/agents/backend-v3/runs/{created.json()['id']}",
        headers=auth_headers(dev_tokens["access_token"]),
    )
    assert response.status_code == 200


async def test_viewer_cannot_run_backend_v3(client):
    owner, owner_tokens = await create_authenticated_user(
    ...
    response = await run_backend_v3(client, viewer_tokens["access_token"], requirement["id"])
    assert response.status_code == 403


async def test_project_manager_can_run_backend_v3(client):
    owner, owner_tokens = await create_authenticated_user(
    ...
    await setup_backend_v3_pipeline(client, pm_tokens["access_token"], requirement["id"])
    response = await run_backend_v3(client, pm_tokens["access_token"], requirement["id"])
    assert response.status_code == 201
...
async def test_validation_failure_marks_run_failed(client):
    ...
    invalid = mock_backend_v3_output()
    data = invalid.model_dump()
    data["generated_files"] = data["generated_files"][:1]
    invalid_output = BackendDeveloperV3Output(**data)
