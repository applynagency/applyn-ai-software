...
from app.performance_test.validator import MINIMUM_COUNTS, PerformanceTestValidator
...
@pytest.fixture
def validator():
    return PerformanceTestValidator()


def test_valid_output_passes(validator):
    result = validator.validate(mock_performance_test_output())
...
def test_empty_output_fails(validator):
    result = validator.validate(PerformanceTestOutput())
