...
from app.agents.security_test import SecurityTestAgent
...
from app.models.security_test import SecurityTestRun, SecurityTestRunStatus
...
from app.repositories.security_test import SecurityTestArtifactRepository, SecurityTestRunRepository
...
from app.security_test.validator import SecurityTestValidator
from app.tenancy.guards import get_requirement_for_org, get_security_test_run_for_org
...
class SecurityTestService:
    def __init__(self, session: AsyncSession):
        ...
        self.run_repo = SecurityTestRunRepository(session)
        self.artifact_repo = SecurityTestArtifactRepository(session)
    ...
    def _to_response(self, run: SecurityTestRun) -> SecurityTestRunResponse:
        artifact = None
        if run.artifacts:
            latest = sorted(run.artifacts, key=lambda item: item.created_at, reverse=True)[0]
            artifact = SecurityTestArtifactResponse.model_validate(latest)
        return SecurityTestRunResponse(
            id=run.id,
    ...
    async def run(
        ...
        security_run, _ = await self.execute_internal(
        ...
        run = await self.run_repo.get_with_artifact(security_run.id)
    ...
    async def execute_internal(
        ...
        *,
    ...
    ) -> tuple[SecurityTestRun, dict]:
        """Run Security Test agent from workflow dispatcher without API permission checks."""
        ...
        run = await self.run_repo.create(
            ...
            status=SecurityTestRunStatus.PENDING,
            created_by=user.id,
        ...
        await self.audit_repo.log(
            action="security_test_started",
            resource_type="security_test_run",
            resource_id=run.id,
            user_id=user.id,
            details={
                "requirement_id": requirement_id,
        ...
        await self.run_repo.update(run, status=SecurityTestRunStatus.RUNNING)
        ...
        try:
            ...
            await self.run_repo.update(
                run,
                status=SecurityTestRunStatus.FAILED,
                completed_at=datetime.now(UTC),
                error_message=str(exc),
            )
            await self.audit_repo.log(
                action="security_test_failed",
                resource_type="security_test_run",
                resource_id=run.id,
                user_id=user.id,
        ...
        if not validation.is_valid:
            await self.audit_repo.log(
                action="security_test_validated",
                resource_type="security_test_run",
                resource_id=run.id,
                user_id=user.id,
            ...
            raise ValidationError(f"Security Test output failed validation (score: {validation.score})")
        ...
        await self.run_repo.update(
            run,
            status=SecurityTestRunStatus.COMPLETED,
    ...
    async def get_run(
        ...
        run = await get_security_test_run_for_org(self.session, run_id, org_context)
        return self._to_response(run)
    ...
    async def get_artifact(
        ...
        if not artifact:
            raise NotFoundError("SecurityTestArtifact", artifact_id)
        return SecurityTestArtifactResponse.model_validate(artifact)
    ...
    async def list_by_requirement(
        ...
        *,
        ...
        return SecurityTestRunListResponse(
