"""Periodic GitOps state sync for verified Argo CD marketplace connections."""
...
class IntegrationGitopsSchedulerRunner:
    async def run_once(self, session: AsyncSession) -> dict:
        ...
        for conn in connections:
            ...
            if not supports_gitops_sync(key):
                skipped += 1
                continue
            ...
            try:
                await IntegrationSyncService(session).sync_connection(
                    user, org_context, conn.id, sync_runs=False,
                )
                synced += 1
                ...
                errors += 1
                logger.info(
                    "integration_gitops_sync_skipped",
...
async def integration_gitops_loop() -> None:
    """Background loop: refresh GitOps application state from Argo CD."""
    from app.core.config import settings
    from app.database.session import AsyncSessionLocal
    from app.observability.tracing import start_as_current_span
    from app.redis.locks import scheduler_lock

    runner = IntegrationGitopsSchedulerRunner()
    interval = max(60, settings.JOB_CRON_INTEGRATION_GITOPS_SYNC_SECONDS)
    logger.info("integration_gitops_scheduler_started", interval_seconds=interval)

    while True:
        try:
            async with scheduler_lock("integration_gitops") as token:
                if token is not None:
                    with start_as_current_span("scheduler.integration_gitops", kind="consumer"):
                        async with AsyncSessionLocal() as session:
                            result = await runner.run_once(session)
                            if result.get("synced"):
                                logger.info("integration_gitops_scheduler_tick", **result)
        except asyncio.CancelledError:
            logger.info("integration_gitops_scheduler_stopped")
            raise
        except Exception as exc:  # pragma: no cover - defensive
            logger.error("integration_gitops_scheduler_tick_failed", error=str(exc))
