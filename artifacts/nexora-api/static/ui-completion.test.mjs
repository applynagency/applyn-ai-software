...
test("UI completion: login page supports SSO providers", () => {
  ...
  const settingsSource = readFileSync(settingsOrgJsPath, "utf8");
  assert.match(appSource, /loadSsoProviders/);
  assert.match(appSource, /auth-sso-providers/);
  assert.match(settingsSource, /\/v1\/auth\/sso\/providers/);
});
...
test("UI completion: renderAuth shows SSO when providers loaded", () => {
  ...
  state.ssoProviders = [
    { slug: "acme", display_name: "Acme SSO", protocol: "OIDC" },
  ];
  const html = renderAuth();
  assert.match(html, /Acme SSO/);
  assert.match(html, /auth-sso-providers/);
});
