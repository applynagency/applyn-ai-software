...
function renderIncidentTimeline() {
  ...
  const timelineEvents = (tl && tl.timeline) || [];
  ...
  return `
    <div class="container">
      ...
      <section class="card">
        ...
        ${timelineEvents.length === 0 ? `<p class="muted">No timeline events recorded.</p>` : `
        <div class="ai-timeline">${timelineEvents.map((e) => `
          <div class="ai-timeline-step">
            <strong style="font-size:12px;">${escapeHtml(e.provider || e.event_type || "event")}</strong>
            <span class="muted" style="font-size:11px;margin-left:8px;">${e.timestamp ? formatDate(e.timestamp) : ""}</span>
            <p class="muted" style="font-size:12px;margin:4px 0 0;">${escapeHtml(truncateIncidentText(e.summary || e.message || "", 400))}</p>
          </div>`).join("")}</div>`}
      </section>
      <section class="card">
